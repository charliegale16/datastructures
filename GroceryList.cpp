...
GroceryList& GroceryList::operator+=(const GroceryList& rhs)
{
	...
	for (GroceryItem gi : rhs._groceries_vector)
	{
		GroceryList::insert(gi);
...
std::size_t GroceryList::find(const GroceryItem& item) const
{
	...
	std::vector<GroceryItem>::iterator it = std::find(_groceries_vector.begin(), _groceries_vector.end(), item);
	return std::distance(_groceries_vector.begin(), it);
...
void GroceryList::insert(const GroceryItem& item, std::size_t offsetFromTop)       // insert new item at offsetFromTop, which places it before the current item at offsetFromTop
{
	...
	/**********  Insert into array  ***********************/
	{
		...
		if (_groceries_array_size >= _groceries_array.size()) // resizing of array
		{
			std::array<GroceryItem, _groceries_array_size * 2> arr;
			std::copy(std::begin(_groceries_array), std::end(_groceries_array), std::begin(arr));
			_groceries_array = arr;
